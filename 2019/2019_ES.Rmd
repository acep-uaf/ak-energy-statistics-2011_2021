...
drive <- "~/Desktop"
setwd(drive)
workbook <- "~/Desktop/workbooks/2019 AK Energy Statistics early release 06Nov2023.xlsx"
...
save_raw <- "~/repos/ak-energy-statistics-2011_2021/2019/raw/"
setwd(save_raw)
...
**VERY IMPORTANT**
```{R}
save_staging <- "~/repos/ak-energy-statistics-2011_2021/2019/staging/"
setwd(save_staging)
...
## table 2.4a
"Generation is net of station service for EIA data and gross of (includes) station service for PCE data"
...
print(`Table 2.4a`[4,1])

# save fifth row as column names
# watch out! different than others
colnames(`Table 2.4a`) <- `Table 2.4a`[6, ]

# delete extra rows
`Table 2.4a` <- `Table 2.4a`[-c(1:6), ]
...
if(`Table 2.5a`[1,1] == "Table 2.5a   Revenue, Sales and Customers by customer type for certificated utilities ($000, MWh, Accounts), 2019") {
  
  # NAs below table are messing things up, delete
  `Table 2.5a` <- `Table 2.5a`[-c(198:208), ]
...
if(`Table 2.5b`[1,1] == "Table 2.5b   Average Annual Energy Use and Rates by Customer Type for certificated utilities (kWh/Customer, $/Customer, $/kWh), 2019"){
  # save second row as column names
...
if(`Table 2.5c`[1,1] == "Table 2.5c   Average Residential Rates and PCE Payments ($/kWh), 2019"){
...
Extra Credit
combine 2.5a, 2.5b and 2.5c
maybe they used to be one table?
join by intertie and community
```{R}
