...
drive <- "~/Desktop"
setwd(drive)
workbook <- "~/Desktop/workbooks/2013 AK Energy Statistics.xlsx"
...
save_raw <- "~/repos/ak-energy-statistics-2011_2021/2013/raw/"
setwd(save_raw)
...
**VERY IMPORTANT**
```{R}
save_staging <- "~/repos/ak-energy-statistics-2011_2021/2013/staging/"
setwd(save_staging)
...
```{R}
# metadata riders and title, print in comments above this chunk, then delete for now
print(`Table 2.4a`[1,1])
print(`Table 2.4a`[2,1])

# save row as column names
# watch out! different than others
colnames(`Table 2.4a`) <- `Table 2.4a`[3, ]

# delete extra rows
`Table 2.4a` <- `Table 2.4a`[-c(1:3), ]
...
## table 2.5a
### "Table 2.5a Revenue, Sales and Customers by Customer Type by Operators/Utilities ($000, MWh, Accounts), 2013"
```{R}
...
## table 2.5b
### "Table 2.5b  Average Annual Energy Use and Rates by Customer Type by Operators/Utilities, (kWh/Customer, $/Customer, $/kWh), 2013"
```{R}
# save title, use to prevent repeat of cleaning expression
title_2.5b <- `Table 2.5b`[1,1]
print(title_2.5b)

if(`Table 2.5b`[1,1] == "Utility Name"){
  # save second row as column names
  colnames(`Table 2.5b`) <- `Table 2.5b`[1, ]
  
  # delete first row (title), second row (blank) and third row (column names)
  `Table 2.5b` <- `Table 2.5b`[-c(1), ]
...
## table 2.5c
### "Table 2.5c  Average Residential Rates and PCE Payments ($/kWh), 2013"
```{R}
# save title
title_2.5c <- `Table 2.5c`[1,1]
print(title_2.5c)

if(`Table 2.5c`[1,1] == "Utility Name"){
  # save third row as column names
  colnames(`Table 2.5c`) <- `Table 2.5c`[1, ]
  
  # delete first row (title), second row (blank), and third row (column names)
  `Table 2.5c` <- `Table 2.5c`[-c(1), ]
}
