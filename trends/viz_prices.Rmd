...
viz %>%
  ggplot() +
    ...
    # coastal mean line
    geom_segment(data=mean_region, aes(x=1.1, xend=1.45,
                                       y=mean_region$mean_weighted_price[1], yend=mean_region$mean_weighted_price[1]),
                                       size=1.2) +
    # railbelt mean line
    geom_segment(data=mean_region, aes(x=2.1, xend=2.43,
                                       y=mean_region$mean_weighted_price[2], yend=mean_region$mean_weighted_price[2]),
                                       size=1.2) +
    # rural remote mean line
    geom_segment(data=mean_region, aes(x=3.1, xend=3.49,
                                       y=mean_region$mean_weighted_price[3], yend=mean_region$mean_weighted_price[3]),
                                       size=1.2) +
    ...
    labs(
         # title = "Price of Residential Electricity, 2011-2019",
...
viz %>%
  ...
  labs(title = element_blank(),
       ...
       caption = "Pop-weighted means, inflation-adjusted to 2021 dollars, post PCE Subsidy",
       x = element_blank(),
       y = "Cents per kWh") +
...
# Rural Remote
   ...
   scale_y_continuous(breaks=seq(0,75,10), limits=c(0,75), expand = expansion(mult = 0)) +
   scale_x_continuous(breaks=seq(2011,2019,1), limits=c(2011,2019), expand = expansion(mult = 0)) 


# write the source data
viz_data_filepath <- gsub(" ", "_", (paste("./trends/visuals/price_sector_", str_to_lower(plot_region), ".csv", sep = "")))
write_csv(viz, file = viz_data_filepath)

# write the visual in EPS
viz_plot_filename <- gsub(" ", "_", (paste("./trends/visuals/price_sector_", str_to_lower(plot_region), ".eps", sep = "")))
ggsave(filename = viz_plot_filename, 
       device = "eps", 
       height = 4,
       width = 6.5,
...
```{R}
consumption <- read_csv("./trends/raw/consumption-per-capita.csv")

long_consumption <- pivot_longer(consumption,
                     cols = c(Coastal, Railbelt, "Rural Remote"),
                     names_to = "Region",
                     values_to = "Consumption")


long_consumption %>%  
  ggplot( aes(y=Consumption, x=Year, color = Region)) +
  geom_line(linewidth=1.5) +
  theme_bw() +
  
  scale_color_manual(values = c("#98BAD6", "#A4CD9A", "#E3918F")) + #Commercial, Other, Residential
  theme(legend.position = "right",
        legend.title = element_blank(),
        legend.text = element_text(size=12),
        axis.text = element_text(size=12),
  ...
  labs(title = element_blank(),
   ...
   scale_y_continuous(breaks=seq(0,10000,1000), limits=c(3000,10000), expand = expansion(mult = 0)) +
   scale_x_continuous(breaks=seq(2011,2019,1), limits=c(2011,2019), expand = expansion(mult = 0)) 
