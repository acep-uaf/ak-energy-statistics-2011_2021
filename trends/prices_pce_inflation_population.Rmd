...
empty_weights <- prices %>%
  ...
  mutate(`industrial_weighted_price` = NA, .after = "Industrial Customers") %>%
  mutate(industrial_weight = NA, .after = "Industrial Customers") %>%
...
for(y in years){
  tmp <- empty_weights %>%
    filter(Year == y)%>%
    mutate(residential_weight = `Residential Customers`/sum(`Residential Customers`, na.rm = T), 
           commercial_weight = `Commercial Customers`/sum(`Commercial Customers`, na.rm = T),
           industrial_weight = `Industrial Customers`/sum(`Industrial Customers`, na.rm = T),
           total_weight = `Total Customers`/sum(`Total Customers`, na.rm = T)
           )
...
for(y in years){
  tmp <- weight_factors %>%
    ...
    mutate(industrial_weighted_price = `Industrial Price kWh (2021 dollars)`*industrial_weight)
